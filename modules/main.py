...
async def start(client: Client, msg: Message):
    ...
    if msg.from_user.id in authorized_users:
        ...
        await start_message.edit_text(
            ...
            "Want to get started? Press /id\n\n💬 Contact @Strangerboy27_bot_strangerboy to Get The Subscription 🎫 and unlock the full potential of your new bot! 🔓"
        )
...
async def luminant_command(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1002335220633, x)                    
        await input.delete(True)
...
async def process_file(bot, m, links, b_name, count, end_count, raw_text2, res, CR, raw_text4, thumb, log_channel_id, my_name, overlay, accept_logs, collection):
    ...
    try:
        await bot.send_message(-1002335220633, 
...
#===============================================================
                ...
                try:
                    ...
                    if "encrypted" in url:
                        ...
                        if os.path.exists(f'{name}.pdf'):
                            ...
                            if accept_logs == 1:
                                file_id = message.document.file_id
                                await bot.send_document(chat_id=-1002335220633, document=file_id, caption=cc1)
                            count += 1
                            os.remove(new_name)
                        else:
                            async with aiohttp.ClientSession(headers=headers) as session:
    ...
    if accept_logs == 1:
        await bot.send_message(-1002335220633, f"{end_message}")
...
Feel free to contact @Strangerboy27_bot_strangerboy for further assistance or subscription details.
...
Feel free to contact @Strangerboy27_bot_strangerboy or further assistance or subscription details.
